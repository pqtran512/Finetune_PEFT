"""
Build Java function-completion training set (HumanEval-Java style).

Mục đích:
  Ghép nhiều nguồn Java → mẫu {prefix, target} giống MultiPL-E / HumanEval-Java:
  - prefix: imports + class Problem + // docs + method signature + "{"
  - target: method body + closing "    }\\n"
  Output: data/java_completion_train.jsonl (dùng bởi main_qwen.py).

Pipeline mỗi mẫu:
  extract public static method → filter Java thật / bỏ C#·Kotlin·JS →
  bỏ method gọi sibling helper → chuẩn hóa indent → dedup →
  decontaminate theo tên method HumanEval-Java.

Nguồn dữ liệu (theo thứ tự trong main):
  1. code-search-net/code_search_net  (Java, function + docstring)     ~25k
  2. bigcode/the-stack-smol           (data/java, gated — cần HF token) ~20k
  3. codeparrot/github-code           (Java-all, streaming)              ~10k
  4. ise-uiuc/Magicoder-OSS-Instruct-75K  (lọc Java từ markdown)        ~8k
  5. nickrosh/Evol-Instruct-Code-80k-v1   (lọc Java từ instruction/out) ~5k

Tham chiếu benchmark (chỉ lấy keyword, không đưa vào train):
  nuprl/MultiPL-E  config=humaneval-java
"""
